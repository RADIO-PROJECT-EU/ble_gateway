...
import org.atlas.wsn.exceptions.WsnErrorCode;
import org.atlas.wsn.exceptions.WsnException;
import org.atlas.wsn.ble.BluetoothBeaconData;
import org.atlas.wsn.ble.listener.AdvertisingReportRecord;
...
public class BluetoothUtil {
     ...
     */
    public static Map<String, String> getConfig(String name) throws WsnException {
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                if (line.toLowerCase().contains("command not found")) {
                    throw new WsnException(WsnErrorCode.OPERATION_NOT_SUPPORTED);
                } else if (line.toLowerCase().contains("no such device")) {
                    throw new WsnException(WsnErrorCode.INTERNAL_ERROR);
                }
            }
            if (br != null) {
                br.close();
