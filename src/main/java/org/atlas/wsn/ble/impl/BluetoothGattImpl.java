...
import org.atlas.wsn.exceptions.WsnTimeoutException;
...
public class BluetoothGattImpl implements BluetoothGatt, BluetoothProcessListener {
    ...
    // --------------------------------------------------------------------
    @Override
    public boolean connect() throws WsnException {
        return connect("hci0");
    }

    @Override
    public boolean connect(String adapterName) throws WsnException {
        this.m_proc = BluetoothUtil.startSession(adapterName, this.m_address, this);
        if (this.m_proc != null) {
        	this.m_bufferedWriter = this.m_proc.getWriter();
            s_logger.debug("Sending connect message...");
            this.m_ready = false;
            ...
            if (!this.m_ready) {
                throw new WsnTimeoutException("Gatttool connection timeout.");
            }
        }
    ...
    public void disconnect() {
        if (this.m_proc != null) {
            ...
            this.m_proc.destroy();
            this.m_proc = null;
            s_logger.debug("Disconnected");
        }
    }

    @Override
    public boolean checkConnection() throws WsnException {
        if (this.m_proc != null) {
            this.m_bufferedWriter = this.m_proc.getWriter();
    ...
    @Override
    public String readCharacteristicValue(String handle) throws WsnException {
        if (this.m_proc != null) {
            this.m_charValue = "";
            ...
            if ("".equals(this.m_charValue)) {
                throw new WsnTimeoutException("Gatttool read timeout.");
            }
            if (this.m_charValue.startsWith("ERROR")) {
                throw WsnException.internalError("Gatttool read error.");
            }
    ...
    @Override
    public String readCharacteristicValueByUuid(UUID uuid) throws WsnException {
        if (this.m_proc != null) {
